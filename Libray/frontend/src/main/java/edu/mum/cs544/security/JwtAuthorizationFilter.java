	...
	private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request, String token) {
		if (StringUtils.isNotEmpty(token)) {
			try {
				...
				List<GrantedAuthority> authorities = ((List<?>) parsedToken.getBody().get(SecurityConstants.JWT_ROLES)).stream()
				...
				if (StringUtils.isNotEmpty(username)) {
					...
					return new UsernamePasswordAuthenticationToken(new ExtensionUserDetails() {
